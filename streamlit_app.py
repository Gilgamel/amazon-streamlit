...
def generate_summary(raw_df, start_date, end_date, region="US"):
    ...
    try:
        ...
        raw_df = raw_df.copy()
        if region == "CA":
            # Force string to handle both ISO8601 and %d.%m.%Y string columns
            raw_df['posted-date'] = raw_df['posted-date'].astype(str)
...
def process_data(file, start_date, end_date, landed_cost_data, pdb_us_data, region="US", tax_report_file=None):
    ...
    try:
        ...
        if region == "CA":
            # Force string to handle both ISO8601 and %d.%m.%Y string columns
            raw_source_df['posted-date'] = raw_source_df['posted-date'].astype(str)
...
if region == "CA":
    ...
    date_col = df_dates['posted-date'].astype(str)
    # Step 1: ISO8601 rows -> convert to %d.%m.%Y string
    iso_mask = date_col.str.contains('T', na=False)
    if iso_mask.any():
